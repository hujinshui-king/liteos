...
public class psCommand implements cmdcontrol {
    ...
    public void handleresponse(String[] options, int optioncount, String [] parameters, int parametercount, ArrayList reply, fileDirectory fdir) {
        ...
        // colorOutput.println(colorOutput.COLOR_BRIGHT_GREEN,  "The returned has " + length + "bytes");
        ...
        while (reply.size() > 0 )
        {
            ...
            try{
            processName = (new String(temparray, "ISO-8859-1")).substring(4 , 4 + msgLength - 4);
            }
            catch  (java.io.UnsupportedEncodingException e) {}
            status = (0x000000FF & ((int) response[start + 3]));
            //System.out.println(processName + "   " + StatusToString(status));
            if (status !=8)
             colorOutput.println(colorOutput.COLOR_BRIGHT_GREEN, processName + "   " + StatusToString(status));
            else
            {
                try{
                processName = (new String(temparray, "ISO-8859-1")).substring(6 , 6  + msgLength - 6);
                }
                 catch  (java.io.UnsupportedEncodingException e) {}
                int addr =    (0x000000FF & ((int) response[start+4]))*256 + (0x000000FF & ((int) response[start+5]));
                addr = addr*2;
                colorOutput.println(colorOutput.COLOR_BRIGHT_GREEN, processName + "   " + StatusToString(status) + " at 0x" + Integer.toHexString( 0x1000000 | addr).substring(1).toUpperCase() );
