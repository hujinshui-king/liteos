...
public class breakpointCommand  {
   ...
   public void removeBreakPointAddr(int addr)
   {
    ...
    private class  breakpointinfo {
           ...
           {
               System.out.println("Breakpoint at 0x"+  Integer.toHexString( 0x1000000 | addr).substring(1).toUpperCase());
           }
   ...
   breakpointinfo [] InternalBreakpoints = new breakpointinfo[128];
    ...
    public int setNewCommand(String[] options, int optioncount, String [] parameters, int parametercount, fileDirectory fdir) {
        ...
        {     System.out.println("Please select a different breakpoing. Cross-page breakpoint is currently disabled.");
              return 0;
        }
    ...
    void updateInternalBreakpoint(int addr, boolean status, byte[] response)
    {
      int i;
        ...
        for (i=0;i<breakpointNum;i++)
        {     if (InternalBreakpoints[i].getAddr() == addr)
          {  if  (status) InternalBreakpoints[i].setEnable();
              else  InternalBreakpoints[i].disable();
   ...
   {
       System.out.println("Note that there may be additional breakpoints on the thread that are not listed.");
    ...
    public void handleresponse(String[] options, int optioncount, String [] parameters, int parametercount, ArrayList reply,fileDirectory fdir) {
       ...
       // for (int start = 0; start + 32 <= length; start += 32) {
            ...
            addr = (unsignedByteToInt(response[start + 3])*256 + unsignedByteToInt(response[start + 4]))*256 +unsignedByteToInt(response[start + 5]);
