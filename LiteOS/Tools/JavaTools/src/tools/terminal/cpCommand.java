...
/**
 * This is the class for implementing the copy command.
 * Copy command is one of the most complicated command in terms of implementation.
 * Here are detailed explanations of how it is implemented
 * <p/>
 * cp supports three models of copy operation
 * <p/>
 * TYPE I
 * cp from the PC to the mote
 * <p/>
 * TYPE II
 * <p/>
 * cp from the mote to the PC
 * <p/>
 * TYPE III
 * cp between two folders on the SAME mote
 * <p/>
 * The support for cp between different motes is not implemented yet.
 * <p/>
 * <p/>
 * cp from the PC to the mote
 * the module issues one command, and waits for one single response. Then it starts to send out multiple
 * commadns in the handle section, where it waits for one more reply and ends
 * <p/>
 * *
 * cp from the mote to PC
 * the module issues one command, and waits for a whole amount of feedback from the mote. This is the most simple
 * <p/>
 * <p/>
 * cp between folders on the same mote
 * the PC sends out two commands, and then waits for one reply
 * <p/>
 * <p/>
 * Here are some more design chocies
 * <p/>
 * Type I
 * gerneate one message, send one message, reply handle, skip one input, generate more messages, send more, handle reply
 * <p/>
 * Type II
 * gerenate one message, send one message, reply handle, skip one input, generate zero message, receive more, handle reply
 * <p/>
 * Type III
 * gerenate two ...
 */
...
public class cpCommand implements cmdcontrol {
    ...
    */
    ...
    public int readFile(String filename, byte[] bytes) {
        ...
        try {
            ...
            File f = new File(filename); // only used to be able to init the ByteArrayoutputStream
        ...
        System.arraycopy(out.toByteArray(), 0, bytes, 0, out.toByteArray().length);
    ...
    public int setNewCommand(String[] options, int optioncount, String [] parameters, int parametercount, fileDirectory fdir) {
        ...
        if (SkipContinue == 1) {
            if (copyType == 1) {
                ...
                //open the file in binary mode and write itsretur contents into the dataCommands as specificied and wait
                //prepare data commands and fill in the info with newnodeid and localFile
                ...
                for (int i = 1; i <= totalseq; i++) {
                    ...
                    for (int j = 0; j < SIZEOFDATA; j++)
                        dataCommands[i - 1][7 + j] = filecontent[(i - 1) * SIZEOFDATA + j];
                }
                if (remain > 0) {
                    int i = totalseq + 1;
                    ...
                    for (int j = 0; j < remain; j++)
                        dataCommands[i - 1][7 + j] = filecontent[remainstart + j];
                }
                int i;
                if (remain > 0)
                    i = totalseq + 2;
    ...
    public int checkreply(int count, byte [] response) {
        ...
        if (!((response[start + 3] == 15) && (response[start + 4] == 15)))
            ...
            return (0x000000FF & ((int) response[start + 5])) * 256 + (0x000000FF & ((int) response[start + 6]));
    }
   ...
   public byte[] getEndPacket()
   {
    ...
    public void handleresponse(String[] options, int optioncount, String [] parameters, int parametercount, int length, byte[] response, fileDirectory fdir) {
        ...
        if ((copyType == 1) && (SkipContinue == 0)) {
           // System.out.println("the total length " + length);
            colorOutput.println(colorOutput.COLOR_BRIGHT_GREEN,  "The total length is "+ length);
        ...
        if ((copyType == 1) && (SkipContinue == 1)) {
            SkipContinue = 0;
           colorOutput.println(colorOutput.COLOR_BRIGHT_GREEN,  "Copy finished");
            return;
        }
        ...
        if ((copyType == 2) && (SkipContinue == 1)) {
            ...
            //use localFile to write the feedback into this file in binary mode, after parsing the data
    ...
    public void handleresponse(String[] options, int optioncount, String [] parameters, int parametercount, ArrayList reply , fileDirectory fdir) {
           ...
           if ((copyType == 1) && (SkipContinue == 0)) {
              // System.out.println("the total length " + length);
               colorOutput.println(colorOutput.COLOR_BRIGHT_GREEN,  "The reply has "+ length + " packets.");
               if (reply.size() == 1)
               {
               ...
               {SkipContinue = 0;
                colorOutput.println(colorOutput.COLOR_BRIGHT_RED,  "The copy command appears to have failed. Please try again.");}
    ...
    public void getIncomingData(ArrayList reply) {
      ...
      /*    int j;
        ...
        {
          colorOutput.print(colorOutput.COLOR_YELLOW, ""+new Byte(response[j]).toString()+" ");
            if ((j+1)%32 == 0)
              System.out.println();
      ...
      */
      ...
      while (reply.size()>0) {
            ...
            response = (byte [])reply.remove(0);
            roundsync = (0x000000FF & ((int) response[start + 4])) * 256 + (0x000000FF & ((int) response[start + 5]));


            if ((((byte) response[start + 3]) == (byte) 239) && (((byte) response[start + 4] == (byte) 239)))
                break;
         ...
         //   System.out.println("Current round is " + roundsync);
             if ((roundsync%30 == 0)&&(roundsync>0))
              colorOutput.println(colorOutput.COLOR_BRIGHT_GREEN,  "Copy round is " + (roundsync-29) + " to "+ roundsync);

            System.arraycopy(response, start, subbytes, 0, 32);
            try {
                incomingDataArray[roundsync] = new String(subbytes, "ISO-8859-1");

            } catch (java.io.UnsupportedEncodingException e) {
            }
        }

        if ((response[start + 3] == (byte) 239) && (response[start + 4] == (byte) 239)) {
            IncomingDataFileSize = (0x000000FF & (int) response[start + 5]) * 256 + ((0x000000FF & (int) response[start + 6]));
            endofFile = true;
    ...
    public void handlePatch(ArrayList reply) {
        ...
        while (reply.size() > 0) {
            ...
            if (response[start + 1] != 125) {
                colorOutput.println(colorOutput.COLOR_BRIGHT_RED,  "Irrelvant reply received. Corrupted Packet.");
                continue;
            }


            if (((0x000000FF & ((int) response[start + 3])) * 256 + (0x000000FF & ((int) response[start + 4]))) != MyCurrentWait * 20)
            //if (((int)response[start+3])*256+(int)response[start+4]!=MyCurrentWait)
            {
               // System.out.println("Incorrect Matching for cp patch");
               colorOutput.println(colorOutput.COLOR_BRIGHT_RED,  "Incorrect Matching for cp patch.");
               Dump.dump(System.out, "data",  response);
                continue;

            }
            //System.out.println("Patch successful for block " + MyCurrentWait);
             colorOutput.println(colorOutput.COLOR_BRIGHT_GREEN,  "Patch successful for block " + MyCurrentWait);
            byte[] subbytes = new byte[32];

            System.arraycopy(response, start, subbytes, 0, 32);
            try {
                incomingDataArray[MyCurrentWait] = new String(subbytes, "ISO-8859-1");
            }
            catch (java.io.UnsupportedEncodingException e) {
    ...
    public int patchresponsecount() {
        ...
        for (i = 0; i < IncomingDataFileSize; i++) {
            if (incomingDataArray[i] == "") {
               colorOutput.println(colorOutput.COLOR_BRIGHT_RED,  "Not fully patched error.");
            }
        }
        ...
        for (i = 0; i < IncomingDataFileSize; i++) {
            ...
            try {
                ...
                colorOutput.println(colorOutput.COLOR_BRIGHT_RED,  "Unsupported Encodings Error.");
            }
    ...
    public ArrayList patchedresponse() {
        ...
        for (i = 0; i < IncomingDataFileSize; i++) {
            ...
            try {
                System.arraycopy(incomingDataArray[i].getBytes("ISO-8859-1"), 6, temp, index, incomingDataArray[i].getBytes("ISO-8859-1")[0] - 6);
