...
/**
 * @author Qing Cao
 *         The terminal class is the entry class of the whole shell system. Basically it operates the serial port to
 *         communicate with the base station, and allows the user to type in commands to do all sorts of operations
 *         on a sensor network.
 *         <p/>
 *         This class uses several other classes, such as the listcommand class, etc, to operate the as proxies for the
 *         user application.
 */
...
public class terminal {
    ...
    //when countCommand = 0, then there is no command needs to be sent over the serial port
    ...
    /**
     * This function inits the file directory. Like all commands, the file directories are
    ...
    /**
     * The main function executes from here. This is the entry point of everything.
     *
     * @param args The standard input arguments. Not used now. May be used in the future.
     * @throws IOException Throws exception only if the I/O encounters a problem.
     */
    public static void main(String [] args) throws IOException {

        //nextcommand is used to determine whether another variable should be read from the console
        ...
        while (nextCommand == true) {
           ...
           if (commandName.compareTo("who") == 0) {
            ...
            if (commandName.compareTo("foreach") == 0) {
                 colorOutput.println(colorOutput.COLOR_YELLOW, "Sorry, Foreach command is not supported in the demo version.");
                 nextCommand = true;
                 SkipReading = 0;
                 continue;
            }
             ...
             if (commandName.compareTo("login") == 0) {
                 colorOutput.println(colorOutput.COLOR_YELLOW, "Sorry, Security commands are not supported in the demo version.");
                 nextCommand = true;
                 SkipReading = 0;
                 continue;
            }
             ...
             if ((commandName.compareTo("quit") == 0)||(commandName.compareTo("exit")==0)) {
                 colorOutput.println(colorOutput.COLOR_YELLOW, "Thank you for using LiteShell. Bye!");
                 System.exit(0);
            }
            ...
            if (commandName.compareTo("list") == 0) {
             ...
             {
                countCommand =  channelhandle.setNewCommand(options, optioncount, parameters, parametercount, fdir);
                if (countCommand > 0) {
            ...
            //From now on, the generic pattern is that the command is got from the respective classes, and the delay
            //is got, and the new commands are copied and sent out

            //THe following segments use both delay of the command as well as its length of data received as the
            //key component of judging whether it should return after finishing the receiving job.

            //Now if the command is ls

            if (commandName.compareTo("ls") == 0) {

                countCommand = lshandle.setNewCommand(options, optioncount, parameters, parametercount, fdir);

                //set up how many commands are used and the timeout of this command
            ...
            //now if the command is cd. the returned number of commands must be 0.

            if (commandName.compareTo("cd") == 0)

            {
                countCommand = cdhandle.setNewCommand(options, optioncount, parameters, parametercount, fdir);
            ...
            if (commandName.compareTo("mv") == 0) {
                //System.out.println("The move command has not been tested. Use cp and rm instead.");
               colorOutput.println(colorOutput.COLOR_BRIGHT_RED, "The move command has not been tested. Use cp and rm instead.");
            }
...

            {
               ...
               // pl.setThreshold(32);
                serverpl.setPacketWaitBuffer(SHELLPORT, 1);
                countCommand = mdhandle.setNewCommand(options, optioncount, parameters, parametercount, fdir);

                if (countCommand > 0) {
                    //pl.setWait(mdhandle.getDelay());
                    serverpl.setPacketWaitTimeout(SHELLPORT, mdhandle.getDelay());
                    for (int i = 0; i < countCommand; i++) {
...

            {
                ...
                //pl.setThreshold(32);
                serverpl.setPacketWaitBuffer(SHELLPORT, 1);
                countCommand = formathandle.setNewCommand(options, optioncount, parameters, parametercount, fdir);
                if (countCommand > 0) {
                  //  pl.setWait(formathandle.getDelay());
                    serverpl.setPacketWaitTimeout(SHELLPORT, formathandle.getDelay());
                    for (int i = 0; i < countCommand; i++) {
            ...
            if (commandName.compareTo("touch") == 0) {
                ...
                countCommand = touchhandle.setNewCommand(options, optioncount, parameters, parametercount, fdir);

                if (countCommand > 0) {
                  //  pl.setWait(touchhandle.getDelay());
                    serverpl.setPacketWaitTimeout(SHELLPORT, touchhandle.getDelay());
            ...
            if (commandName.compareTo("rm") == 0) {
                ...
                countCommand = rmhandle.setNewCommand(options, optioncount, parameters, parametercount, fdir);
                if (countCommand > 0) {
                //    pl.setWait(rmhandle.getDelay());
                    serverpl.setPacketWaitTimeout(SHELLPORT, rmhandle.getDelay());
            ...
            if ((commandName.compareTo("uninstall") == 0)||(commandName.compareTo("kill")==0)) {

                //pl.setThreshold(32);
                serverpl.setPacketWaitBuffer(SHELLPORT, 1);

                countCommand = uninstallhandle.setNewCommand(options, optioncount, parameters, parametercount, fdir);
                if (countCommand > 0) {
                    //pl.setWait(uninstallhandle.getDelay());
                    serverpl.setPacketWaitTimeout(SHELLPORT, uninstallhandle.getDelay());
                    for (int i = 0; i < countCommand; i++) {
            ...
            if (commandName.compareTo("du") == 0) {
               // pl.setThreshold(32);
                serverpl.setPacketWaitBuffer(SHELLPORT, 1);
                countCommand = duhandle.setNewCommand(options, optioncount, parameters, parametercount, fdir);

                if (countCommand > 0) {
                    //pl.setWait(duhandle.getDelay());
                    serverpl.setPacketWaitTimeout(SHELLPORT, duhandle.getDelay());
                    for (int i = 0; i < countCommand; i++) {
            ...
            if (commandName.compareTo("resetnetwork") == 0) {
               serverpl.setPacketWaitBuffer(SHELLPORT, 1);
                countCommand = resethandle.setNewCommand(options, optioncount, parameters, parametercount, fdir);

                if (countCommand > 0) {
                   // pl.setWait(resethandle.getDelay());
                    serverpl.setPacketWaitTimeout(SHELLPORT, resethandle.getDelay());
                    for (int i = 0; i < countCommand; i++) {
            ...
            if (commandName.compareTo("rbb") == 0) {
                ...
                countCommand = rbbhandle.setNewCommand(options, optioncount, parameters, parametercount, fdir);

                if (countCommand > 0) {
                    //pl.setWait(rbbhandle.getDelay());
                    serverpl.setPacketWaitTimeout(SHELLPORT, rbbhandle.getDelay());
            ...
            if (commandName.compareTo("ps") == 0) {
             //   pl.setThreshold(32);
                ...
                countCommand = pshandle.setNewCommand(options, optioncount, parameters, parametercount, fdir);

                if (countCommand > 0) {
                    //pl.setWait(pshandle.getDelay());
                    serverpl.setPacketWaitTimeout(SHELLPORT, pshandle.getDelay());
            ...
            if (commandName.compareTo("rbn") == 0) {
                ...
                countCommand = rbnhandle.setNewCommand(options, optioncount, parameters, parametercount, fdir);

                if (countCommand > 0) {
                   // pl.setWait(rbnhandle.getDelay());

                    serverpl.setPacketWaitBuffer(SHELLPORT, rbnhandle.getDelay());
            ...
            if (commandName.compareTo("print") == 0) {
                ...
                countCommand = printhandle.setNewCommand(options, optioncount, parameters, parametercount, fdir, debughandle);
            ...
            if (commandName.compareTo("set") == 0) {
                ...
                countCommand = sethandle.setNewCommand(options, optioncount, parameters, parametercount, fdir, debughandle);

                if (countCommand > 0) {
                   // pl.setWait(printhandle.getDelay());
                    serverpl.setPacketWaitTimeout(SHELLPORT, sethandle.getDelay());
                    for (int i = 0; i < countCommand; i++) {
            ...
            if (commandName.compareTo("continue") == 0) {
             ...
             if (commandName.compareTo("snapshot") == 0) {
                         // pl.setThreshold(32);
                          serverpl.setPacketWaitBuffer(SHELLPORT, 1);
                          countCommand =  snapshothandle.setNewCommand(options, optioncount, parameters, parametercount, fdir);

                          if (countCommand > 0) {
                            //pl.setWait(snapshothandle.getDelay());
                              serverpl.setPacketWaitTimeout(SHELLPORT, snapshothandle.getDelay());
                            for (int i = 0; i < countCommand; i++) {
                ...
                if (commandName.compareTo("restore") == 0) {
                        //  pl.setThreshold(32);
                     serverpl.setPacketWaitBuffer(SHELLPORT, 1);
                          countCommand =  restorehandle.setNewCommand(options, optioncount, parameters, parametercount, fdir);
            ...
            if ((commandName.compareTo("find") == 0) || (commandName.compareTo("search") == 0)) {
                countCommand = searchhandle.setNewCommand(options, optioncount, parameters, parametercount, fdir);
                if (countCommand > 0) {
                    //pl.setWait(searchhandle.getDelay());
                    serverpl.setPacketWaitTimeout(SHELLPORT, searchhandle.getDelay());
                    for (int i = 0; i < countCommand; i++) {
            ...
            if ((commandName.compareTo("install") == 0)||(commandName.compareTo("exec")==0)) {

                countCommand = installhandle.setNewCommand(options, optioncount, parameters, parametercount, fdir);

                if (countCommand > 0) {
                  //  pl.setWait(installhandle.getDelay());
                    serverpl.setPacketWaitTimeout(SHELLPORT, installhandle.getDelay());
            ...
            if ((commandName.compareTo("man") == 0)) {
                ...
                if (parametercount == 0)
                {
                    colorOutput.println(colorOutput.COLOR_YELLOW,  "Syntax error for man command");
                     nextCommand = true;
                     SkipReading = 0;
                     continue;

                }
                  manhandle.setNewCommand(options, optioncount, parameters, parametercount, fdir);
           ...
           if (commandName.startsWith("./"))
           {
               ...
               optioncount = 0;
               options[0] = new String(commandName.getBytes(), 2, commandName.length()-2);
               countCommand = devhandle.setNewCommand(options, optioncount, parameters, parametercount, fdir);
               //pl.setThreshold(devhandle.getThreshold());
             //  pl.setThreshold(32);
               serverpl.setPacketWaitBuffer(SHELLPORT, 1);
               //serverpl.setPacketWaitTimeout(SHELLPORT, devhandle.getThreshold());

                if (countCommand > 0) {
                //   pl.setWait(devhandle.getDelay());
                    serverpl.setPacketWaitTimeout(SHELLPORT, devhandle.getDelay());
            ...
            if (countCommand == 1) {
             ...
             //   response = pl.getData();
                ...
                if ((commandName.compareTo("cp") == 0)) {
                    ...
                    //System.out.println("Now count command is larger than 0 with the second round must!");
                    //send N packets and SYNC, and wait to see if ack comes
                    //if no ack, then retransmit the N packets. Otherwise,
                    //if NACK comes, adjust and retransmit another bulk of packets
                    ...
                    //colorOutput.println(colorOutput.COLOR_BRIGHT_RED, "the count command is "+countCommand);
                    ...
                    while (true) {
                        ...
                        if (currentIndex >= countCommand - 1) {
                            roundofTry = 0;
                            //System.out.println("currentIndex is " + currentIndex + " larger than command num, exit");
                        //     colorOutput.println(colorOutput.COLOR_BRIGHT_RED, "currentIndex is " + currentIndex + " larger than command num, exit");
                        ...
                        if (roundofTry == MAXROUNDOFTRY - 2) {
                            ...
                            if (roundofTry == MAXROUNDOFTRY - 1) {
                                ...
                                //System.out.println("round of retry is " + roundofTry + " try to system reboot ");
                                colorOutput.println(colorOutput.COLOR_BRIGHT_RED, "Logical error of the copy command. Please report this problem to the developer list.");
                                System.exit(0);
                               //this part should not happen!


                                rbnhandle.setNewCommand(options, optioncount, parameters, parametercount, fdir);
                        ...
                        colorOutput.println(colorOutput.COLOR_YELLOW, "Now trying to send " + currentIndex + " " + nextIndex);

                        for (i = currentIndex; i < nextIndex; i++) {
                           //System.out.println("Now trying to send " + currentIndex + " " + nextIndex);
                        ...
                        //removed so that at the end of the delivery there should be a sync
                    ...
                    //    response = pl.getData();
                        ...
                        if (responseList.size() == 0) {
                            ...
                            colorOutput.println(colorOutput.COLOR_BRIGHT_RED ,   "No sync reply");
                            roundofTry++;
                         ...
                         //   pl.read();
                            ...
                            if ((tempreply = cphandle.checkreply(responsecount, response)) == 0) {
                                //System.out.println("Now reply is good on sync");

                                colorOutput.println(colorOutput.COLOR_YELLOW, "Now reply is good on sync");
                                currentIndex = nextIndex;
                                roundofTry = 0;
                            } else if (tempreply == -1) {
                                //System.out.println("Wrong sync reply");
                                colorOutput.println(colorOutput.COLOR_BRIGHT_RED, "Wrong sync reply");
                                roundofTry++;
                            //    pl.write(rbbhandle.getNewCommand(0));
                             //   pl.read();
                                continue;
                                ...
                                //System.out.println("Sync reply shows has got " + tempreply);

                                colorOutput.println(colorOutput.COLOR_YELLOW,  "Sync reply shows has got " + tempreply);
                                //pl.write(rbbhandle.getNewCommand(0));
                                //pl.read();
                    ...
                    if (roundofTry > 0) {
                        //System.out.println("cp failed because of lack of back signal");
                        colorOutput.println(colorOutput.COLOR_BRIGHT_RED,  "cp failed because of lack of back signal");
                    } else {
                        //System.out.println("cp succeed");
                        colorOutput.println(colorOutput.COLOR_BRIGHT_GREEN,  "cp succeed");
                ...
                if ((commandName.compareTo("cp") == 0)) {
                   ...
                   serverpl.setPacketWaitTimeout(SHELLPORT, 50);
                    ...
                    //System.out.println("Now Patching");
                     colorOutput.println(colorOutput.COLOR_YELLOW,     "Now Patching");
            ...
            //if countCommand >0, the results must have been received and should be analyzed to see the results
...

            {
                ...
                if (commandName.compareTo("ls") == 0) {
                    ...
                    lshandle.handleresponse(options, optioncount, parameters, parametercount, responseList, fdir);
                }
                ...
                if (commandName.compareTo("cp") == 0) {
                    ...
                    cphandle.handleresponse(options, optioncount, parameters, parametercount, responseList, fdir);
                    SkipReading = cphandle.skipReading();
                   // colorOutput.println(colorOutput.COLOR_BRIGHT_RED, "this time is "+SkipReading);
               ...
               if (commandName.startsWith("./") == true) {
                    ...
                    devhandle.handleresponse(options, optioncount, parameters, parametercount, responseList, fdir);
                ...
                if (commandName.compareTo("mkdir") == 0) {
                    mdhandle.handleresponse(options, optioncount, parameters, parametercount, responseList, fdir);
                }
                ...
                if (commandName.compareTo("touch") == 0) {
                    //touchhandle.handleresponse(options, optioncount, parameters, parametercount, responsecount, response, fdir);
                    touchhandle.handleresponse(options, optioncount, parameters, parametercount, responseList, fdir);
                }

                if (commandName.compareTo("rm") == 0) {
                    rmhandle.handleresponse(options, optioncount, parameters, parametercount, responseList, fdir);
                }

                if (commandName.compareTo("du") == 0) {
                    //if (responsecount==0)
                    // colorOutput.println(colorOutput.COLOR_RED,  "There is no info packet received.");
                    duhandle.handleresponse(options, optioncount, parameters, parametercount, responseList, fdir);
                }
                if ((commandName.compareTo("search") == 0) || (commandName.compareTo("find") == 0)) {
                    searchhandle.handleresponse(options, optioncount, parameters, parametercount, responseList, fdir);
                }
                ...
                if (((commandName.compareTo("install") == 0))||(commandName.compareTo("exec")==0)) {
                    installhandle.handleresponse(options, optioncount, parameters, parametercount, responseList, fdir);
                }

                if (((commandName.compareTo("uninstall") == 0))||(commandName.compareTo("kill")==0)) {
                    uninstallhandle.handleresponse(options, optioncount, parameters, parametercount, responseList, fdir);
                }

                if ((commandName.compareTo("rbb") == 0)) {
                    rbbhandle.handleresponse(options, optioncount, parameters, parametercount, responseList, fdir);
                }

                if ((commandName.compareTo("rbn") == 0)) {
                    rbnhandle.handleresponse(options, optioncount, parameters, parametercount, responseList, fdir);
                }

                if ((commandName.compareTo("print") == 0)) {
                                   printhandle.handleresponse(options, optioncount, parameters, parametercount, responseList, fdir, debughandle);
                }

                if ((commandName.compareTo("breakpoint") == 0)) {
                                   breakpointhandle.handleresponse(options, optioncount, parameters, parametercount, responseList, fdir);
                }

                  if (commandName.compareTo("continue") == 0) {
                   continuehandle.handleresponse(options, optioncount, parameters, parametercount, responseList, fdir);
                }
            } else if (countCommand == 0) {
                ...
                if (commandName.compareTo("snapshot") == 0) {
                                  ...
                                  snapshothandle.handleresponse(options, optioncount, parameters, parametercount, responseList, fdir);
              }


                if (commandName.compareTo("restore") == 0) {

                                  restorehandle.handleresponse(options, optioncount, parameters, parametercount, responseList, fdir);
              }
            ...
            if (SkipReading == 0) {
                ...
                colorOutput.println(colorOutput.COLOR_YELLOW,  "Time elapes " + aftertime + " \n");
            }

        }

        //System.out.println("shell exit");
         colorOutput.println(colorOutput.COLOR_BRIGHT_GREEN,  "Time elapes " + aftertime + " \n");
    }
}
