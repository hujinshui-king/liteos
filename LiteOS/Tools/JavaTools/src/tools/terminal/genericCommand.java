...
public class genericCommand {
    ...
    static boolean commandValidCheck(String commandName, String[] options, int optioncount, String [] parameters, int parametercount, fileDirectory fdir)
    {
        ...
        if (commandName.compareTo("debug") == 0)
                    {
                     ...
                     if (dirname.startsWith("/")!=true)
                             colorOutput.println(colorOutput.COLOR_YELLOW, "The directory format is error. Must starting with /. For example, /c/Source/App");
                     ...
                     if ((a.exists() == false )||(b.exists()==false))
                     {
                       ...
                       b = new File(dirname+"\\bin\\LiteOS.elf");
                      if ((a.exists() == false )||(b.exists()==false))
                     {
         ...
         if (commandName.compareTo("setchannel") == 0)
              {
                  if ((optioncount == 0) && (parametercount == 1))
                  {
                      ...
                      {  colorOutput.println(colorOutput.COLOR_BRIGHT_RED, "The parameter must be an integer.");
                         return false;
                      }
                  }
                   else
        ...
        if (commandName.compareTo("mv") == 0)
         ...
         {
             if ((optioncount == 0) && (parametercount == 1))
            { if ((fdir.getCurrentNode().getType().compareTo("network")== 0)||(fdir.getCurrentNode().getType().compareTo("root")== 0))
                {    return false;}
                 else
                 {   return true;}
             }
          ...
          if (commandName.startsWith("./") == true)
         {
             if ((optioncount == 0) && (parametercount > 0))
             {
                  ...
                  { colorOutput.println(colorOutput.COLOR_BRIGHT_RED, "You are at invalid directory to invoke device commands.");
                    return false;
                  }
                 }
             else
