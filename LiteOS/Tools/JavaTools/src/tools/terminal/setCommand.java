    ...
    public void handleresponse(String[] options, int optioncount, String [] parameters, int parametercount, ArrayList reply ,fileDirectory fdir, debugCommand debughandle) {
       ...
       // for (int start = 0; start + 32 <= length; start += 32) {
        while (reply.size() > 0)
        {
          ...
          //  System.arraycopy(response, start + 3, targetData, 0, response[start + 0] - 3);
          //  break;
        }

        if (setsuccess == true)
        colorOutput.print(colorOutput.COLOR_BRIGHT_GREEN,  "The set command is successful.");
        else
        colorOutput.print(colorOutput.COLOR_BRIGHT_RED,  "The set command may not be successful.");
