...
public class PreferenceModel extends AbstractTreeTableModel {
	...
	private void createTree(Preferences parentPrefs,
			PreferenceTreeItem treeParent) {
		...
		try {
			...
			for (int i = 0; i < childNames.length; i++) {
				...
				PreferenceTreeItem childTree = new PreferenceTreeItem(
						childPrefs, parent);
			...
			for (int i = 0; i < keyNames.length; i++) {
				PreferenceAttribute prefAttr = new PreferenceAttribute(
						parentPrefs, parent, keyNames[i], parentPrefs.get(
								keyNames[i], ""));
	...
	public class PreferenceTreeItem {
		...
		public Element getXMLElement(boolean deep) {
			...
			if (deep) {
				Iterator childIt = childs.iterator();
				while (childIt.hasNext()) {
					Element childElem = ((PreferenceTreeItem) childIt.next())
							.getXMLElement(deep);
					if (childElem != null)
