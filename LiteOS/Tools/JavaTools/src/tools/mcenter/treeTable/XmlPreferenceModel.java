package tools.mcenter.treeTable;
...
public class XmlPreferenceModel extends AbstractTreeTableModel {
	...
	public XmlPreferenceModel(Document xmlDoc) {
		...
		itemTreeRoot = new XmlPreferenceModel.PreferenceRoot(prefs, null,
				elementRoot.getName());
	...
	public void setValueAt(Object aValue, Object node, int column) {
		if (column == 2) {
			((PreferenceTreeItem) node).setSelected(((Boolean) aValue)
					.booleanValue(), true);
			this.jTreeTable.tableChanged(new javax.swing.event.TableModelEvent(
					jTreeTable.getModel(), 0, jTreeTable.getRowCount(), 2));
		}
	}

	private void createXmlTree(Preferences parentPrefs,
			PreferenceTreeItem treeParent, Element parentElement) {
		...
		while (childIt.hasNext()) {
			...
			PreferenceTreeItem childTree = new PreferenceTreeItem(parentPrefs,
					treeParent, childElement.getAttributeValue("name"));
		...
		while (childIt.hasNext()) {
			Element childElement = (Element) childIt.next();
			PreferenceAttribute prefAttr = new PreferenceAttribute(parentPrefs,
					treeParent, childElement.getAttributeValue("key"),
	...
	public class PreferenceTreeItem {
		...
		public PreferenceTreeItem(Preferences parentPrefs,
				PreferenceTreeItem parent, String prefsNodeName) {
		...
		public void setSelected(boolean selected, boolean deep) {
			if (deep) {
				...
				while (childIt.hasNext()) {
					((PreferenceTreeItem) childIt.next()).setSelected(selected,
							deep);
				}
			}
		...
		public void registerPreference(boolean deep, Preferences parentPrefs) {
			...
			String separator = (parentPrefs.absolutePath().endsWith("/")) ? ""
					: "/";
			Preferences childPref = parentPrefs.node(parentPrefs.absolutePath()
					+ separator + prefsNodeName);
			if (deep) {
				Iterator childIt = childs.iterator();
				while (childIt.hasNext()) {
					((PreferenceTreeItem) childIt.next()).registerPreference(
							deep, childPref);
	...
	public class PreferenceRoot extends PreferenceTreeItem {
		...
		public PreferenceRoot(Preferences parentPrefs,
				PreferenceTreeItem parent, String prefsNodeName) {
	...
	public class PreferenceAttribute extends PreferenceTreeItem {
		...
		public PreferenceAttribute(Preferences parentPrefs,
				PreferenceTreeItem parent, String key, String value) {
		...
		public String toString(int column) {
			...
			case 1:
				return value;
			case 2:
				return new String("" + this.selected);
