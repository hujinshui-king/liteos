...
void reply_devicehandling(uint8_t * receivebuffer)
{
    ...
    ramstart =
        (uint8_t *) fsread16uint(newblockid, DEV_PROGRAMENTRYOFFSET + 2);
    if (paramlength > 2)
    {
        mystrncpy((char *)ramstart, (char *)&receivebuffer[paramindex +
                                                           countparam + 1],
                  paramlength - countparam - 2);
        ramstart[paramlength - countparam - 1] = '\0';
    }
    ramreturn = (uint16_t *) fsread16uint(newblockid, DEV_PROGRAMENTRYOFFSET +
                                          4);
    entryfunc = (void (*)(void))fsread16uint(newblockid,
                                             DEV_PROGRAMENTRYOFFSET);
    if (looproundcount == 0)
    {
        looproundcount = 1;
    }
...
void reply_memory(uint8_t * receivebuffer)
{
    ...
    for (i = 0; i < LITE_MAX_THREADS; i++)
    {
        if (thread_table[i].state != STATE_NULL)
        {
            len = mystrlen((char *)thread_table[i].threadName);
            reply[7] = len;
            mystrncpy((char *)&reply[8], (char *)thread_table[i].threadName,
                      len);
            reply[8 + len] = 0;
...
{
    create_thread(createtaskthread, createramstart, createstackpointer,
                  createstaticdatasize, 1, (char *)createthreadname,
                  createflashromstart, createflashromsize);
    //energyround = 0; 
}
...
void create_thread_task()
{
    ...
    pageno = hex2value(sysinfo[0]) * 100 + hex2value(sysinfo[1]) * 10
        + hex2value(sysinfo[2]);
    entryaddr =
        (uint16_t) (hex2value(sysinfo[3])) * 4096 +
        (uint16_t) (hex2value(sysinfo[4])) * 256 +
        (uint16_t) (hex2value(sysinfo[5])) * 16 +
        (uint16_t) (hex2value(sysinfo[6]));
    ramstackstart =
        (uint16_t) (hex2value(sysinfo[7])) * 256 +
        (uint16_t) (hex2value(sysinfo[8])) * 16 +
        (uint16_t) (hex2value(sysinfo[9]));
    ramstackend =
