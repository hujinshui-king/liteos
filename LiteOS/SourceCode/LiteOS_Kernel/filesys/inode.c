...
void buildDeviceDirectory()
{
...
#ifdef PLATFORM_AVR
    {
        ...
        buildDeviceNode(newaddr, "led", set_led_task,
                        (uint8_t *) adc_parameter,
                        (uint8_t *) adc_returnvalue);
        buildDeviceNode(newaddr, "light", get_light_task,
                        (uint8_t *) adc_parameter,
                        (uint8_t *) adc_returnvalue);
        buildDeviceNode(newaddr, "temp", get_temp_task,
                        (uint8_t *) adc_parameter,
                        (uint8_t *) adc_returnvalue);
        buildDeviceNode(newaddr, "magnet", get_mag_task_x,
                        (uint8_t *) adc_parameter,
                        (uint8_t *) adc_returnvalue);
        buildDeviceNode(newaddr, "accel", get_acc_task_x,
                        (uint8_t *) adc_parameter,
                        (uint8_t *) adc_returnvalue);
        buildDeviceNode(newaddr, "radio",
                        SocketSendRadioThroughCommandandAdcTask,
                        (uint8_t *) adc_parameter,
                        (uint8_t *) adc_returnvalue);
    }
#endif
...
//-------------------------------------------------------------------------
void buildDeviceNode(uint8_t parentnode, char *name, void (*entryfunc) (void),
                     uint8_t * ramparameterstart, uint8_t * ramreturnstart)
{
    uint8_t newaddr;
    uint16_t temp;
