...
__z void hal_frame_read( hal_rx_frame_t *rx_frame ){
    ...
    if ((frame_length >= HAL_MIN_FRAME_LENGTH) && (frame_length <= HAL_MAX_FRAME_LENGTH)) {
        ...
        *rx_data++ = frame_length; 
		...
		tat_do_ed_scan(&ed_level);
		
		rx_frame->rssi = ed_level; 
 ...
 *
 * @return The system time with symbol resolution.
 *
 */
...
ISR( TIMER1_CAPT_vect ){
    ...
    if ((interrupt_source & HAL_RX_START_MASK)) {
        ...
        //Handle Interupt to See if Tx or Rx End
...
/** @brief This function receives the data from the radio. 
