...
/* utility function to perform the changes in the hex file according to different node id*/

void reAddress(uint8_t *ptr, int nodeid) {

	ptr[0] = 0x80 + nodeid%16;
	ptr[1] = 0xe0 + nodeid/16; 
}


/* Utility function to write an Intel HEX8 record to a file */
int Write_IHexRecord(IHexRecord *ihexRecord, FILE *out, int addr, int nodeid) {
	int i;
	

	int addr_even = (int)(addr/16)*16; 
	/* Check our record pointer and file pointer */
	...
	if ((ihexRecord->address == addr_even) && (oncedone == 0))
	{
	   reAddress(&(ihexRecord->data[addr - addr_even]), nodeid);
	   /* now recalculates the checksum for later use */
	   ihexRecord->checksum = Checksum_IHexRecord(ihexRecord);
